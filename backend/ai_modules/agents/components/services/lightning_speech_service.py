...
if __name__ == "__main__":
    ...
    voice_module = LightningSpeechService()
    response = voice_module.text_to_speech("Today is February 26, 2025, and the current time is 18:43:58. Now, regarding the weather forecast for the next few days in Hanoi, today you can expect patchy rain with a maximum temperature of 17.3 degrees Celsius and a minimum of 15 degrees")
    
    if isinstance(response, bytes):
        if response[:4] == b'RIFF' and response[8:12] == b'WAVE':
            print("Valid WAV file")
        else:
            print("Invalid WAV file")
    
    speaker.play_audio(response)