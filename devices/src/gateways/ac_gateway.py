...
class ACGateway:
    def __init__(self, ac_devices: List[ACDevice]):
        self.ac_devices = {ac_devices.device_id: ac_devices for ac_devices in ac_devices} 
        
    def get_device(self, device_id: str) -> Union[ACDevice, None]:
        return self.ac_devices.get(device_id)
    
    def get_all_devices(self) -> List[ACDevice]:
    ...
    def log_ac_status(self, id: Union[int]):
        ac_id = 'ac_' + str(id)
    ...
    def get_temperature_by_id(self, id: Union[int]) -> float:
        ac_id = 'ac_' + str(id)
        if ac_id in self.ac_devices:
            return self.ac_devices[ac_id].get_temperature(id)
        return 0
    ...
    def get_fan_speed_by_id(self, id: Union[int]) -> int:
        ac_id = 'ac_' + str(id)
        if ac_id in self.ac_devices:
            return self.ac_devices[ac_id].get_fan_speed(id)
        return 0
    ...
    def get_humidity_by_id(self, id: Union[int]) -> int:
        ac_id = 'ac_' + str(id)
        if ac_id in self.ac_devices:
            return self.ac_devices[ac_id].get_humidity(id)
        return 0
    ...
    def get_mode_by_id(self, id: Union[int]) -> str:
        ac_id = 'ac_' + str(id)
        if ac_id in self.ac_devices:
            return self.ac_devices[ac_id].get_mode(id)
        return None
    
    def get_ac_status(self) -> List[bool]:
        return [ac.state == "on" for ac in self.ac_devices.values()]
    
    def get_ac_by_id(self, id: int) -> ACDevice:
        ac_id = 'ac_' + str(id)
        if ac_id in self.ac_devices:
            return self.ac_devices[ac_id]
        return None
    
    def adjust_temp_for_all(self, level: float):
